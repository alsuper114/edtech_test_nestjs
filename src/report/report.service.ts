...
export class ReportService {
    ...
    public async getReport(userId: number): Promise<any> {
        ...
        const redisResult = await this.cacheManager.get(`GET: /api/v1/report/${userId}`);
        ...
        await Promise.all(Object.keys(lessons).map(async (lessonId) => {
            let quizes = (await this.lessonService.getQuizesByLessonId(Number(lessonId))).quizes;
            quizes = await Promise.all(quizes.map(async (quiz:Quize) => {
                let score = 0;
        ...
        user.lessonUserNotes.forEach((luNote) => {
            lessons[luNote.lessonId].note.push({content: luNote.note, position: luNote.position});
        })

        await this.cacheManager.set(`GET: /api/v1/report/${userId}`, lessons, 10000);
        return lessons;
    }
}
